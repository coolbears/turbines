...
dashboardSidebar( actionButton("go", "Change color") ),
  sidebarLayout(
    sidebarPanel(
...
server <- function(input, output, session) {
  ...
  output$distPlot <- renderPlot({
    ...
    if(input$selectvar == 1 & input$checkbox5 == TRUE ){
      hist(turbines$Turbine.Capacity, breaks = input$bins,xlim=c(input$col, 0),main='Distribution of Turbine Capacity',xlab='Turbine Capacity (KW)',col = 'purple', border = 'darkgrey')
    }
    else if(input$selectvar == 1 & input$checkbox6 == TRUE ){
      hist(turbines$Turbine.Capacity, breaks = input$bins,xlim=c(input$col,0), main='Distribution of Turbine Capacity',xlab='Turbine Capacity (KW)',col = 'green', border = 'darkgrey')
    }
    
    if(input$selectvar == 2 & input$checkbox5 == TRUE){
      hist(turbines$Turbine.Hub_Height, breaks = input$bins,xlim=c(input$col,0), main='Distribution of Turbine Hub Heights',xlab='Turbine Hub Heights (KM)',col = 'purple', border = 'darkgrey')
      
    }
    else if (input$selectvar == 2 & input$checkbox6 == TRUE){
      hist(turbines$Turbine.Hub_Height, breaks = input$bins,xlim=c(input$col,0), main='Distribution of Turbine Hub Heights',xlab= 'Turbine Hub Heights (KM)',col = 'green', border = 'darkgrey')
      
    }
    if(input$selectvar == 3 & input$checkbox5 == TRUE ){
      hist(turbines$Project.Capacity, breaks = input$bins,xlim=c(input$col,0), main='Distribution of Project Capacity',xlab='Project Capacity (KW)',col = 'purple', border = 'darkgrey')
      
    }
    else if (input$selectvar == 3 & input$checkbox6 == TRUE ){
      hist(turbines$Project.Capacity, breaks = input$bins, xlim=c(input$col,0),main='Distribution of Project Capacity',xlab='Project Capacity (KW)',col = 'green', border = 'darkgrey')
      
    }
    if(input$selectvar == 4 & input$checkbox5  == TRUE ){
      hist(turbines$Project.Number_Turbines, breaks = input$bins,xlim=c(input$col,0), main='Distribution Number of Turbines',xlab='Number of Turbines',col = 'purple', border = 'darkgrey')
      
    }
    else if(input$selectvar == 4 & input$checkbox6  == TRUE ){
      hist(turbines$Project.Number_Turbines, breaks = input$bins, xlim=c(input$col,0),main='Distribution Number of Turbines',xlab='Number of Turbines',col = 'green', border = 'darkgrey')
      
    }
    if(input$selectvar == 5 & input$checkbox5  == TRUE ){
      barplot(table(turbines$Site.State),main='Frequency of Turbines According to State',xlab='States',xlim=c(input$col,0),ylab='Frequency of Turbines',ylim=c(0,20000),col=c('aquamarine','purple'))
      
    }
    else if (input$selectvar == 5 & input$checkbox6  == TRUE ){
      barplot(table(turbines$Site.State),main='Frequency of Turbines According to State',xlab='States',xlim=c(input$col,0),ylab='Frequency of Turbines ',ylim=c(0,20000),col=c('aquamarine','green'))
  ...
  observeEvent(input$go, {
    insertUI("head", "beforeEnd", 
             tags$style(HTML('.content-wrapper, .left-side { background-color: green; }'))
    )
